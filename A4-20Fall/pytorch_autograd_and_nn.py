...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(
      nn.Conv2d(C, channel_1, kernel_size=kernel_size_1, stride=1, padding=pad_size_1),
      nn.ReLU(),
      nn.Conv2d(channel_1, channel_2, kernel_size=kernel_size_2, stride=1, padding=pad_size_2),
      nn.ReLU(),
      Flatten(),
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    layers.append(nn.Conv2d(Cin, Cout, kernel_size=3, stride=stride, padding=1))
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample or Cin != Cout:
      stride = 2 if downsample else 1
      self.shortcut = nn.Conv2d(Cin, Cout, kernel_size=1, stride=stride, padding=0)
    else:
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for i in range(len(stage_args)):
      Cin_stage = stage_args[i-1][1] if i > 0 else stage_args[0][0]
      Cout_stage, num_blocks, downsample = stage_args[i][1:]
      layers.append(ResNetStage(Cin_stage, Cout_stage, num_blocks, downsample, block))
  ...
  def forward(self, x):
    ...
    scores = self.fc(F.adaptive_avg_pool2d(self.cnn(x), (1, 1)).squeeze(-1).squeeze(-1))
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    layers.append(nn.BatchNorm2d(Cin))
    layers.append(nn.ReLU())
    layers.append(nn.Conv2d(Cin, mid_channels, kernel_size=1, stride=1, padding=0))
    layers.append(nn.BatchNorm2d(mid_channels))
    layers.append(nn.ReLU())
    layers.append(nn.Conv2d(mid_channels, mid_channels, kernel_size=3, stride=stride, padding=1))
    layers.append(nn.BatchNorm2d(mid_channels))
    layers.append(nn.ReLU())
    layers.append(nn.Conv2d(mid_channels, Cout, kernel_size=1, stride=1, padding=0))
